...
*/
vector<bool> kidsCandies(vector<int> candies, int extra){
    vector<bool> ans(candies.size(), true);
    int max_val = *max_element(candies.begin(), candies.end());
    for (int i=0; i<candies.size(); i++){
        if ((candies[i] + extra) < max_val) ans[i] = false;
    }
    return ans;
}
...
/*
~/github/ComputerScience/Problems$ ./1431_Kids_With_st_Number_of_Candies 
